...
class UtilisateurController extends Controller
{
    function creerUtilisateur(Request $request )
    {
        if (Utilisateur::query()->where('email', $request->email)->first()) {
            ...
            $contient->idinventaire = $inventaire->idinventaire;
            $contient ->iditem = 1;
            $contient->save();
...
public function equiperItem(Request $request, $idutilisateur)
{
    if (Auth::check()) {
        $utilisateurAuth = Auth::user();
        $accessToken = $utilisateurAuth->currentAccessToken();
        ...
        if ($accessToken && $accessToken->tokenable_id == $idutilisateur) {
            $utilisateur = Utilisateur::find($idutilisateur);
            $utilisateur->iditem = $request->iditem;
            $utilisateur->save();
            return response()->json($utilisateur->iditem);
        } else {
